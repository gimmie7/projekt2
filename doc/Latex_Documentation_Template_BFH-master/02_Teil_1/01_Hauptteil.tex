...
\label{sec:teil1_kombinatorik}
Die zuvor diskutierten Ansätze weisen, wie schon im vorherigen Kapitel erwähnt, noch einige Probleme auf und sind für eine präzise Disaggregation somit nur bedingt geeignet. Es wird ein Modell benötigt, welches robust ist, auch wenn die Anzahl der Verbraucher erhöht wird. Ausserdem sollten auch Multistatus-Verbraucher klassifiziert werden können, wie beispielsweise ein Staubsauger oder eine Waschmaschine. Und auch Verbraucher mit ähnlichem Verbrauch sollte das Modell erkennen und unterscheiden können. Deep Learning könnte ein geeigneter Ansatz dafür sein. Deep Learning ist eigentlich ein Überbegriff für eine Reihe von maschinellen Lerntechniken. In Neuronalen Netzen bezeichnet es Netzwerke mit vielen Schichten. Das Hauptziel der Verwendung dieser Art von Architektur besteht darin, eine Hierarchie von Features zu lernen, bei der jede einzelne Schicht die Eingaben für sich verarbeitet und die daraus resultierenden Ausgabedaten in Form von verbesserten Eingabedaten für die nächste Schicht bereitstellt. Deep Learning-Modelle sind sehr attraktiv, weil sie flexibel sind, das heisst, ähnliche Modelle können in einer Vielzahl von verschiedenen Problemen verwendet werden. Sie sind in der Lage, automatisch neue Feature-Repräsentationen zu erlernen, die in der Vergangenheit für jedes unterschiedliche Problem von Hand erstellt wurden.

Um NILM mittels Neuronalen Netzen abzubilden, werden allerdings sehr viele Daten benötigt. Trainingsdatensätze zu diesem Thema existieren aber schon, einer davon ist REDD. Dieser ist öffentlich verfügbar mit Daten über den Verbrauch von 10 Häusern in der USA, wo das System zweiphasig ist. Es enthält gesampelte Daten in niedriger Frequenz (1 Hz) für das ganze Haus und einzelne Verbraucher. Im Bereich Deep Learning unterscheidet man zwischen drei verschiedenen Lernmethoden:

\textbf{Supervised Learning:}  
Beim überwachten Lernen wird das Modell nicht nur mit dem aggregierten Verbrauch trainiert, sondern auch mit dem Verbrauch der einzelnen Verbraucher. Somit müssen Daten auf Verbraucherebene gesammelt werden, die dann als Trainingsdaten für das Modell verwendet werden.
